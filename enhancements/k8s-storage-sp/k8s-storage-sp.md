...
The v1 implementation focuses exclusively on PVC lifecycle management
(CREATE, READ, UPDATE, DELETE). UPDATE supports capacity expansion when the
StorageClass allows it. StorageClass provisioning, volume snapshots, and
cross-cluster migration are out of scope. The K8s Storage SP implements the
`storage` service type schema and runs one provider instance per managed
Kubernetes cluster.
...
- Manage `PersistentVolumeClaim` resources per cluster (one SP deployment per
  cluster).
...
- One SP process per managed cluster. Each SP instance is configured with a
  single kubeconfig (or in-cluster service account). A single SP process does
...
- Local development: uses the `k8s-storage` profile in
  [api-gateway](https://github.com/dcm-project/api-gateway) compose (parallel to
  `k8s-container`).
...
- Creates and watches `PersistentVolumeClaim` resources. Each storage request
  becomes one PVC with capacity, access mode, and optional StorageClass from
...
| defaultAccessMode   | string | ReadWriteOnce     | Access mode when not specified in request       |

Users can override `accessMode` via the top-level request field. `storageClass`
and `volumeMode` may be set per volume under
...
The POST endpoint follows the portable `storage` service type contract.
Required fields: `capacity`, `metadata.name`. Optional: `accessMode`,
`providerHints.kubernetes`.
...
**StorageClass via providerHints:**
...
**Pre-patch validation (v1):** The SP checks policy preconditions only. It does
**not** pre-flight backend free space, cloud account quotas, or Ceph pool
capacity — those are not exposed through a portable Kubernetes API. Expansion
may still fail asynchronously after a successful PATCH.

Optional: if the namespace has a `ResourceQuota` limiting
`requests.storage`, the SP may reject the PATCH when the new total would exceed
quota (best-effort; not a substitute for backend capacity checks).
...
Whether expansion completes **online** (while the Pod is running) depends on
the CSI driver, `volumeMode` (`Filesystem` vs `Block`), and mount state — not
only `allowVolumeExpansion` on the StorageClass.
...
| Namespace `ResourceQuota` (optional)  | Whether filesystem resize needs Pod restart |
...
See [Volume Expansion](https://kubernetes.io/docs/concepts/storage/persistent-volumes/#expanding-persistent-volumes-claims)
...
resources labeled with `managed-by=dcm` and `dcm-service-type=storage`. When a
relevant PVC event occurs, the SP maps Kubernetes state to DCM generic status
and publishes a CloudEvent to the messaging system.
...
- The `instanceId` is read from the `dcm-instance-id` label on the PVC.
- When bound, the status message may include `volumeName` from
  `pvc.spec.volumeName`.

### Test Plan

- **Unit tests**: Request validation, label application, status mapping logic.
- **Integration tests**: Against KIND or Minikube with a default StorageClass;
  verify CREATE → Bound → READ → PATCH (expand) → DELETE lifecycle when
  `allowVolumeExpansion` is enabled.
- **Status reporting tests**: Confirm CloudEvents published on Pending → Bound
  transitions.
- **E2E**: Catalog item instance for `serviceType: storage` through Placement
  Manager to K8s Storage SP in dev stack (`api-gateway` compose profile).
...
- Requires SP to have cluster-wide PVC create permissions or dynamic
...
- `api-gateway` compose profile: `k8s-storage` (parallel to `k8s-container`).
